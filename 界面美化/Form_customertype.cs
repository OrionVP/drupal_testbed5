...
namespace 界面美化
{
    public partial class Form_customertype : Form
    {
        public Form_customertype()
        {
            InitializeComponent();
        }
        ...
        {
            refresh(sender, e);
            String sql = "delete from 客户种类";
            SqlConnection conn = new SqlConnection(str);
            conn.Open();
            SqlCommand mycom = new SqlCommand(sql, conn);
            mycom.ExecuteNonQuery();
            conn.Close();
            String sql1 = "insert into 客户种类 values('";
            for (int i = 0; i <dataGridView1.RowCount; i++)
            {
                if (i >= 1)
                    sql1 += ",('";
                sql1 += dataGridView1.Rows[i].Cells[0].Value.ToString() + "','所有客户')";
            }
            ...
            conn1.Close();
            Form4_customer fc;
            fc = (Form4_customer)this.Owner;
            fc.refresh();
            //更新系统日志
            String logsql = "insert into 系统日志 values('" + Login.User.name + "','" + "对客户种类进行操作" + "','" + DateTime.Now + "')";
